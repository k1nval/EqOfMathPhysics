namespace ProblemSolver.Solvers
{
    ...
    public class ParabolicSplittingSolver : ISolver<TwoDLayer>
    {
        ...
        private TwoDLayer Next2(TwoDLayer firstLayer, TwoDLayer secondLayer)
        {
            ...
            for (int i = 0; i <= I; i++)
            {
                thirdLayer[i, 0] = problem.Psi(i * h, 0 * h, n * tau);
            }

            // задняя грань
            for (int i = 0; i <= I; i++)
            {
                thirdLayer[i, J] = problem.Psi(i * h, J * h, n * tau);
            }

            // левая грань
            for (int i = 0; i <= J; i++)
            {
                thirdLayer[0, i] = problem.Psi(0 * h, i * h, n * tau);
            }

            // правая грань
            for (int i = 0; i <= J; i++)
            {
                thirdLayer[I, i] = problem.Psi(I * h, i * h, n * tau);
            }
