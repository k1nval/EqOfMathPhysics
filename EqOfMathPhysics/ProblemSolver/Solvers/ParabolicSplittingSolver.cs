    ...
    public class ParabolicSplittingSolver : ISolver<TwoDLayer>
    {
        ...
        public TwoDLayer Solve(int needLayer)
        {
            ...
            for (int i = 1; i <= needLayer; i++)
            {
                ...
                firstLayer = Next1(firstLayer); // v1

                secondLayer = Next2(firstLayer /*v1*/, predLayer /*v0*/);
            }
        ...
        {
            var r = tau / (h * h);
            return firstLayer[i, j] + (r * (secondLayer[i + 1, j] - (2 * secondLayer[i, j]) + secondLayer[i, j - 1]));
        }
